...
macro_rules! cmd {
    ...
    ("waj", $sub_command:literal, $($args:expr),*) => {{
        let waj_bin = env!("CARGO_BIN_EXE_waj");
        let mut command = std::process::Command::new(&waj_bin);
...
#[macro_export]
macro_rules! temp_waj {
    ($name:ident) => {
        temp_waj!($name, "test.waj")
    };
    ($name:ident, $filename:literal) => {
        ...
        let $name = tmp_waj_dir.path().join($filename);
    };
}
